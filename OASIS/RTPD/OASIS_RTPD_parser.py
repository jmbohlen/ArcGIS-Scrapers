...
# This script is run every hour at XX:22 to obtain the third and fourth interval data for the following hour and at XX:47 to obtain the first two intervals .
...
# RTPD
# Parse for first two intervals if current time is >= XX:47
if int(dt.datetime.utcnow().strftime("%M")) >= 47:
    ...
    # Remove the old interval 1+2 data
    cols_to_remove = []
    ...
    # Save a copy of the new DF to a csv
    RTPD_updated_df.to_csv('/var/www/caiso/RTPD_Prices.csv')
...
# Repeat the code above for the 3+4 intervals
...
# Since this data is parsed at odd times through the hour, I have an additional script "arcgis_update.py" that posts the data to ArcGIS at XX00, XX15, XX30, and XX45.
