...
current_gdf = current_gdf.dropna()
current_gdf.to_file('RTD_data.json', driver='GeoJSON')

# prices_gdf = gpd.read_file('/var/www/caiso/past_RTD_data.json', driver='GeoJSON')
current_minute = dt.datetime.now().strftime('%M')
...
try:
    ...
    past_node_locs_flc = past_node_locs[1]
    past_price_flayer_collection = FeatureLayerCollection.fromitem(past_node_locs_flc)
past_price_flayer_collection.manager.overwrite('RTD_data.json')
